...
step/task 内容质量硬要求：同一个课程内每个 step.description/explanation、action、check 必须不同，必须紧扣当前 step title、phase name 和用户目标。禁止复用模板句，尤其禁止“把抽象目标变成一个能立刻执行的小动作”“先照着示例做一遍”“再独立做一遍”“最后记录卡住的位置”“不用看教程也能复现”“并知道失败原因”“它是理解课程导入的关键抓手”“学习时请同时记住含义、使用场景和一个自己的例子”。每一步都必须包含具体动作、学习材料或操作对象、输出成果、验收标准；如果字段缺失也不要用同一段兜底话术。
...
durationWeeks 建议 1-2；phases 3-5 个，每个 phase 就是一张核心步骤卡；每个 phase 要有 practice、checkpoint、commonMistakes、output；每个 phase steps 1-2 步；每个 step 必须包含 explanation/action/check/example，解释要短而具体，强调“马上怎么做”。resources 3 个以内，projects 1-2 个小练习。slides 和 mindMap 可以给最简版本，但不要把 lite 写成完整课程课件。`;
  ...
  return `当前用户选择的生成模式是：深度 AILINES AI 规划 / 系统学习课程 mode=deep。你必须严格遵守这个模式，不允许根据主题看似简单、目标长度、图片内容或自己的判断降级为 lite。系统学习课程要更完整；durationWeeks 建议 8-12；phases 4-6 个，目标适合拆细时可以 6-8 个；每个阶段 topics 4-8 个、steps 3-5 步；每个 step 必须有 explanation/example/action/check；explanation 至少 150-250 中文字；slides 约 8-12 张；mindMap 更完整；必须包含 practice、checkpoint、commonMistakes、output；resources 4-6 个，projects 3 个。`;
}
